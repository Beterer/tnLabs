...
namespace GB.tnLabs.Web.APIControllers
{
    ...
    public class TrainingsController : ApiController
    {
        ...
        [HttpGet]
        public IQueryable<Identity> Trainers()
        {
            var subscriptionIdentityRoles =
                _repository.SubscriptionIdentityRoles.Where(x => x.SubscriptionId == UnitOfWork.ActiveSubscriptionId &&
                                                                 (x.Role == RoleTypes.Owner || x.Role == RoleTypes.Trainer));
            if (subscriptionIdentityRoles.Any())
            {
                var identities = subscriptionIdentityRoles.Select(i => i.IdentityId).Distinct();
                var q = _repository.Identities.Where(x => identities.ToList().Contains(x.IdentityId)).ToList();
                if (identities.Any())
                    return _repository.Identities.Where(x => identities.ToList().Contains(x.IdentityId));
            }
            return new List<Identity>().AsQueryable();
